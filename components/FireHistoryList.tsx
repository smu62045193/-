...
interface FireHistoryListProps {
  isKeywordPopupMode?: boolean;
}

const FireHistoryList: React.FC<FireHistoryListProps> = ({ isKeywordPopupMode = false }) => {
  ...
  const openIndependentWindow = () => {
    ...
    url.searchParams.set('popup', 'fire_contractor');
  ...
  const handleSaveKeywords = async () => {
    ...
    if (success) {
      setSaveStatus('saved');
      if (window.opener) {
        window.opener.postMessage({ type: 'FIRE_KEYWORDS_SAVED' }, '*');
      }
          ...
          <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-slate-900 text-white shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-blue-300 font-bold uppercase tracking-widest">Fire Inspection Contractor Setup</p>
              </div>
            </div>
